package com.zrk.session.config;
...
public class CacheConfiguration {
    ...
    public CacheManager memcachedCacheManager(
        ...
        for (CacheFactory cache : caches) {
            SSMCache ssmCache = new SSMCache(
                cache.getObject(),
                jHipsterProperties.getCache().getMemcached().getExpiration(),
                false
            );
    ...
    private CacheFactory createCache(String cacheName, JHipsterProperties jHipsterProperties) {
        ...
        if (jHipsterProperties.getCache().getMemcached().getAuthentication().isEnabled()) {
            InetSocketAddress memcachedServers = AddrUtil.getOneAddress(
                jHipsterProperties.getCache().getMemcached().getServers()
            );
        ...
        cacheConfiguration.setUseBinaryProtocol(
            jHipsterProperties.getCache().getMemcached().isUseBinaryProtocol()
        );
